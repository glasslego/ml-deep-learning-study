...
def example_5_visualization():
    ...
    y1 = softmax(np.array([x_range, np.zeros_like(x_range), np.zeros_like(x_range)]).T)[:, 0]
    ...
    bars = plt.bar(['클래스 1', '클래스 2', '클래스 3'], probs_viz, color=['#FF6B6B', '#4ECDC4', '#45B7D1'])
